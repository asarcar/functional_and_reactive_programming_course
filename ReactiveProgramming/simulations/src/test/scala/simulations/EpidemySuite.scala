...
class EpidemySuite extends FunSuite {
  ...
  test("vaccine & prevalence rate"){
    val es = new EpidemySimulator

    assert(
      (es.persons.count(_.infected) ==
        es.SimConfig.population * es.SimConfig.prevRate) &
      (es.persons.count(_.vaccinated) ==
        es.SimConfig.population * es.SimConfig.vaccineRate),
      s"Population ${es.SimConfig.population} " +
      s"Infected ${es.persons.count(_.infected)}: " +
      s"Vaccinated ${es.persons.count(_.vaccinated)} " +
      s"prevalence rate should be exactly ${es.SimConfig.prevRate} " +
      s"vaccination rate should be exactly ${es.SimConfig.vaccineRate}")
  }
  ...
  test("life cycle"){
    ...
    assert(es.agenda.head.time == dieTime,
      "You should set a 'die' event (decides with a probability 25% " +
        "whether the person dies) after 14 days")
  ...
  test("transmissibility rate"){
    ...
    assert(infectedTimes > 0,
      "A person should get infected according to the transmissibility " +
        "rate when he moves into a room with an infectious person")
  }
}
