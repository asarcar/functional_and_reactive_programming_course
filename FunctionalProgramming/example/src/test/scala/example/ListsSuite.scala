...
class ListsSuite extends FunSuite {
   ...
   */
  test("sum of a few numbers: 1, 2, 0") {
    assert(sum(List(1,2,0)) === 3)
  }
  
  test("max of a few numbers: 3, 7, 3") {
    assert(max(List(3, 7, 2)) === 7)
  }

  test("max empty list throws java.util.NoSuchElementException exception") {
    intercept[java.util.NoSuchElementException] {
      max(List())
    }
  }

  test("sum empty list throws java.util.NoSuchElementException exception") {
    intercept[java.util.NoSuchElementException] {
      sum(List())
    }
  }

  test("sum of -ve numbers: -2, -1, -3") {
    assert(sum(List(-2, -1, -3)) === -6)
  }

  test("max of -ve numbers: -2, -1, -3") {
