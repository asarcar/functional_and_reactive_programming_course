...
// On the JVM the threads can have the daemon status.
// If all the threads in the active state are daemon threads, the JVM will exit.
// The default ExecutionContext in Scala sets its worker threads to be daemons.
// By default, SBT does not spawn a new JVM process to run your Server
// application. When SBT's is done executing main, it simply returns to the
// interactive shell.
// To kill the non-daemon threads, enable forking when running tasks in SBT
// Note: if we do not run in the same process as SBT, the readLine
// (userInput routine) immediately returns with a value of null.
// fork in run := true
